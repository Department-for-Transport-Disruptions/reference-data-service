...
import { formattedServiceExpected, serviceDbData } from "./test/testdata";
...
describe("get-service-by-id", () => {
    describe("input generation", () => {
        ...
        it("throws a ClientError if invalid serviceId provided", () => {
            ...
            expect(() => getQueryInput(event)).toThrowError("Provided serviceId is not valid");
        });
    });
    ...
    describe("format service", () => {
        it("correctly formats db response", () => {
            ...
            expect(formattedService).toEqual(formattedServiceExpected);
        });
    });
});
